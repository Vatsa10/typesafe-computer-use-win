...
def _utf8_stdout() -> None:
    """Screen text is not ASCII, and a redirected Windows console still defaults to cp1252,
    which raises on the first box-drawing character a payload prints."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    _prepare()
    if args.act and not windows.accessibility_trusted():
        sys.exit(windows.PERMISSION_HINT)
    writer = make_writer()
    if writer is None:
