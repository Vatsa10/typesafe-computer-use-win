...
def app(*children):
    ...
    return node("AXApplication", label="Explorer", frame=(0.0, 1117.0, 0.0, 0.0), children=children)
...
def test_a_frameless_root_does_not_prune_the_whole_tree():
    assert labels(node("AXApplication", "Explorer", frame=None, children=[node("AXButton", "Share")]))[0] == ["Share"]
...
def test_ax_items_convert_points_to_capture_pixels_and_name_the_role(screen, monkeypatch):
    ...
    monkeypatch.setattr(windows, "actionable_elements", lambda pid, w, h: (nodes, [], False))
...
def test_offscreen_controls_are_deduplicated_and_never_repeat_a_visible_item(screen, monkeypatch):
    ...
    monkeypatch.setattr(windows, "actionable_elements", lambda pid, w, h: ([], hidden, False))
...
def test_offscreen_controls_are_empty_in_replay(screen, monkeypatch):
    monkeypatch.setattr(windows, "actionable_elements", lambda pid, w, h: pytest.fail("no pid to walk"))
